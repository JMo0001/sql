...
--20230719
--  T R I G G E R
set serveroutput on; -- 스크립트 창에서 나온다.
...
insert into employee values(905, '길성춘', '690512-1151432', 'gil_sj@kh.or.kr',
                            '01035464455','D5','J3','S5',3000000, 0.1,200,sysdate,
                            null, default);
delete from employee where emp_id = 905;

create table product(pcode number, pname varchar2(30),
                        brand varchar2(30), price number, 
                        stock number default 0,
                        constraints product_pk primary key(pcode)
                    );
create table pro_detail(decode number primary key, pcode number, pdate date,
                        amount number, status varchar2(10),
                        foreign key (pcode) references product(pcode)
                        );
alter table pro_detail rename constraint SYS_C008661 to pro_detail_pk;   
alter table pro_detail add constraint status check (status in ('입고','출고'));
